...
import { authApi, resolveWsOrigin } from '@/utils/api'
...
function getWsUrl(): string {
  ...
  //
  // resolveWsOrigin() falls back to the current page origin in production
  // rather than localhost — a localhost default would make the browser dial
  // the user's own machine.
  const origin = resolveWsOrigin()
  const host = process.env.NEXT_PUBLIC_WS_HOST
  if (host) {
    const proto = origin.startsWith('wss') ? 'wss' : 'ws'
