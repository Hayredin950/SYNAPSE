...
/**
 * Resolve the backend origin for calls that bypass the axios client and the
 * Next.js rewrite proxy — SSE streams and WebSockets, which need an absolute
 * URL.
 *
 * In production an unset NEXT_PUBLIC_API_URL must NOT fall back to localhost:
 * the browser would try to connect to the *user's own machine* and fail with a
 * confusing refused-connection error. Returning '' instead yields a relative
 * URL that routes through the Next.js proxy, which works.
 */
...
/** Same as resolveApiOrigin but as a ws:// or wss:// origin. */
export const resolveWsOrigin = (): string => {
  const origin = resolveApiOrigin()
  if (origin) return origin.replace(/^http/, 'ws')
  if (typeof window === 'undefined') return ''
  // Relative: derive from the current page so wss is used on https pages.
...
const clearTokens = () => {
  ...
  // Also reset Zustand persisted auth state so the login page doesn't
  // redirect back to /home when isAuthenticated is still true in the store.
  try {
    const raw = localStorage.getItem('synapse-auth')
    if (raw) {
      const parsed = JSON.parse(raw)
...
api.interceptors.response.use(
  ...
  async (
    ...
    // Quota exceeded (403 with error_code = quota_exceeded)
    if (error.response?.status === 403) {
      const data = error.response.data as Record<string, unknown> | undefined
      if (data?.error_code === 'quota_exceeded') {
        // Fire via a custom DOM event so we don't couple api.ts to React context
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('synapse:quota_exceeded', { detail: data }))
        }
      }
    }

    // TASK-501-F1: Rate limit exceeded (429) → show banner with countdown
    if (error.response?.status === 429) {
      const data = error.response.data as Record<string, unknown> | undefined
      const resetAt = (data?.reset_at as string) ?? null
      const message = (data?.message as string) ?? 'Rate limit reached. Please try again shortly.'
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('synapse:rate_limit_exceeded', {
            detail: { resetAt, message, data },
          }),
        )
      }
    }
