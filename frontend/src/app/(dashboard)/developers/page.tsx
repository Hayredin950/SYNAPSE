...
// Shown in copy-paste API snippets. Falls back to the documented public API
// host rather than localhost, since these are examples users copy elsewhere.
const BASE_URL =
  process.env.NEXT_PUBLIC_API_URL ||
  (typeof window !== 'undefined' ? window.location.origin : 'https://your-synapse-host');
        ...
        <div className="px-6 mt-10 max-w-4xl space-y-12">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Burst limit</th>
                    <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Daily</th>
                    <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Monthly</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {[
                    { res: 'API requests', burst: '1000 / hour', daily: '—',  monthly: '—'   },
                    { res: 'AI chat',      burst: '50 / day',    daily: '40', monthly: '300' },
                    { res: 'Agent runs',   burst: '10 / day',    daily: '5',  monthly: '30'  },
                    { res: 'Documents',    burst: '—',           daily: '10', monthly: '50'  },
                  ].map(row => (
                    <tr key={row.res} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                      <td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-200">{row.res}</td>
                      <td className="px-4 py-3 font-mono text-xs text-slate-700 dark:text-slate-200">{row.burst}</td>
                      <td className="px-4 py-3 font-mono text-xs text-slate-700 dark:text-slate-200">{row.daily}</td>
                      <td className="px-4 py-3 font-mono text-xs text-slate-700 dark:text-slate-200">{row.monthly}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <p className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-slate-700">
                SYNAPSE is free — every account gets the same limits. Referring others
                raises your monthly ceiling; see <a href="/account" className="underline">Usage &amp; Referrals</a>.
              </p>
            </div>
          </section>
          ...
          <section className="flex flex-wrap gap-4">
            {[
              ...
              { href: '/account', label: '📊 Usage & Referrals', desc: 'See your quota and raise it' },
            ].map(link => (
              link.external ? (
