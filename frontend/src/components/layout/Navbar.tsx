...
import { Search, Sun, Moon, Bell, Menu, LogOut, Settings, User, Check, Trash2, Gift, Zap, Command, MessageSquare } from 'lucide-react'
...
function QuotaBadge() {
  const { data } = useQuery({
    queryKey: ['growth-usage'],
    queryFn: () => api.get('/growth/usage/').then(r => r.data),
    staleTime: 5 * 60_000,
    retry: false,
  })

  // Agent runs are the scarcest resource (each fans out into many LLM calls),
  // so that's the number worth surfacing in the nav.
  const daily = data?.usage?.agent_runs?.daily
  if (!daily || daily.unlimited) return null

  const left = Math.max(0, daily.limit - daily.used)
  const cls =
    left === 0
  ...
  return (
    <Link
      href="/account"
      className={`hidden sm:inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold tracking-wide transition-opacity hover:opacity-80 ${cls}`}
      title="Usage & referrals"
    >
      <Zap size={10} />
      {left} LEFT
    </Link>
            ...
            {isUserMenuOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-card overflow-hidden z-50">
                ...
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                >
                  <Gift size={16} /><span className="text-sm">Usage &amp; Referrals</span>
                </Link>
                <button
