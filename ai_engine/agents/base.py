...
class SynapseAgent:
    ...
    #
    # MAX_ITERATIONS was 10. Each iteration is a separate LLM call, so a single
    # agent run could consume 10-20 upstream requests — enough for a handful of
    # users to exhaust a day's free-tier quota. Empirically ReAct loops converge
    # in 3-4 iterations; the tail mostly burns quota without improving answers.
    # Override per-agent via the max_iterations kwarg when a task genuinely
    # needs more depth.
    ...
    def _build_llm(self):
        ...
        """
        from ai_engine.agents.llm_factory import build_llm_with_fallbacks
