...
try:
    ...
    print(
        f"[celery] WARNING: Could not apply langchain patch (langchain-core not available): {e}"
    )
except Exception as e:
    # Unexpected error — log it but don't crash (might be optional)
    print(
        f"[celery] WARNING: Unexpected error while patching langchain: {e}",
        file=sys.stderr,
    )
# ─────────────────────────────────────────────────────────────────────────────
import os
...
CELERY_BEAT_SCHEDULE = {
    ...
    # ── Semantic cache housekeeping — hourly ─────────────────────────────────
    # Expired entries would otherwise accumulate and slow the ANN lookup.
    "purge-semantic-cache-hourly": {
        "task": "apps.core.tasks.purge_semantic_cache",
        "schedule": crontab(minute=17),  # off the hour to avoid a thundering herd
        "options": {"queue": "default"},
    },
}
