...
@shared_task(
    bind=True, max_retries=3, name="apps.users.tasks.create_initial_workflows_task"
)
def create_initial_workflows_task(self, user_id: str, prefs_id: str):
    ...
    """
    logger.info(
        f"[ONBOARDING] Starting create_initial_workflows_task for user {user_id}"
    )
    try:
        ...
        logger.info(
            f"[ONBOARDING] Successfully created initial workflows for user {user.email}"
        )
        ...
        logger.error(
            f"[ONBOARDING] OnboardingPreferences {prefs_id} not found for onboarding"
        )
        return {"success": False, "error": "Preferences not found"}
    except Exception as exc:
        logger.exception(
            f"[ONBOARDING] Failed to create initial workflows for user {user_id}: {exc}"
        )
