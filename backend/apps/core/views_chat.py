...
class _OpenRouterDirectPipeline:
    ...
    def chat(
        ...
        # Semantic cache — only for the FIRST turn of a conversation.
        # A follow-up ("what about the second one?") means something different
        # depending on what preceded it, so a history-independent cache would
        # serve wrong answers. First turns are self-contained, and they are
        # where the duplicate-topic overlap actually is: many users ask about
        # the same trending subject on the same day.
        # Skipped when files are attached — the answer depends on them.
        cacheable = not history and not files
        if cacheable:
            from apps.core.semantic_cache import lookup  # noqa: PLC0415

            cached = lookup(question, scope="chat")
        ...
        if cacheable and answer:
            from apps.core.semantic_cache import store  # noqa: PLC0415

            store(question, answer, scope="chat")
