...
@shared_task(name="apps.core.tasks.purge_semantic_cache")
def purge_semantic_cache() -> dict:
    """
    Delete semantic-cache entries past their TTL.

    Without this the cache grows without bound, which both wastes disk and
    slows the ANN lookup that makes the cache worth having.
    """
    from apps.core.semantic_cache import purge_expired
