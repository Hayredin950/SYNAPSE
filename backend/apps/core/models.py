...
class SemanticCacheEntry(models.Model):
    """
    Cached LLM response, keyed by the embedding of the prompt.

    Lets a paraphrased question reuse an earlier answer instead of spending a
    fresh LLM call. See apps.core.semantic_cache for the lookup logic and the
    similarity threshold.
    """

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    # Partitions the cache (e.g. "chat" vs "agent") so responses of one shape
    # are never served in place of another.
    scope = models.CharField(max_length=64, default="default", db_index=True)
    prompt = models.TextField(max_length=4000)
    prompt_embedding = VectorField(dimensions=1024)
    ...
    def __str__(self):
        return f"[{self.scope}] {self.prompt[:60]}"
